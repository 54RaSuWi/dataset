...
class HeartDisease(BaseModel):
    ...
    gender: int = Query(..., description="Jenis kelamin anda (0 = wanita, 1 = pria)")
    ...
    fbs: int = Query(..., description="Kadar gula (0 = kurang dari sama 120 mg/dl, 1 = lebih dari 120 mg/dl)")
...
def predict():
    ...
    ca = st.selectbox("Jumlah pembuluh darah besar yang terdeteksi", [0, 1, 2, 3])
    ...
    if st.button("predict"):
        ...
        print("Predicting.....")
        prediction = model_last.predict(data.reshape(-1, 1))
        if prediction == 1:
            prediction = 'Pasien mempunyai risiko terkena penyakit jantung'                    
        else:
            prediction = 'Pasien tidak mempunyai risiko terkena penyakit jantung'
        return {
